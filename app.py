...
def validate_api_token(token):
    """Validate the API token format and existence"""
    if not token or token.strip() == '':
        st.error("""
        Please enter your Apify API token.
        You can get it from https://console.apify.com/account/integrations
        """)
        st.stop()
        return False
    return True
...
def initialize_apify_client():
    """Initialize Apify client with user-provided API token"""
    # Display API token input in sidebar
